...
const dispatchEvents = function(optimistic, mongoConnection, collectionName, channels, events) {
    if (optimistic) {
    ...
    Meteor.defer(() => {
        ...
        events.forEach(event => {
            ...
            const docId = event[RedisPipe.DOC]._id;
            const dedicatedChannel = getDedicatedChannel(mongoConnection, collectionName, docId);
...
const dispatchUpdate = function(
    ...
    dispatchEvents(optimistic, mongoConnection, collectionName, channels, events);
};

const dispatchRemove = function(optimistic, mongoConnection, collectionName, channels, docs) {
    const uid = optimistic ? RedisSubscriptionManager.uid : null;
...
const dispatchInsert = function(optimistic, mongoConnection, collectionName, channels, doc) {
    const uid = optimistic ? RedisSubscriptionManager.uid : null;
    ...
    dispatchEvents(optimistic, mongoConnection, collectionName, channels, [event]);
};
